...
import { PhoneIncoming, Clock, Download, ChevronRight, Search } from 'lucide-react';
...
export default function AdminCallsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await adminApi.getCalls({ search, status });
  ...
  const openTranscript = async (call: any) => {
    setSelectedCall(call);
    try {
      const res = await adminApi.getCallTranscript(call.id);
      setTranscript(res.data.transcript || []);
    } catch { setTranscript([]); }
  };

  const downloadRecording = async (callId: string) => {
    const res = await adminApi.getCallRecording(callId);
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-[#8888aa] text-sm mt-1">{total} total calls recorded</p>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative">
          ...
          <input className="input pl-10 w-64" placeholder="Search by number…"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 table-container">
          <table className="table">
            ...
            <tbody>
              {loading
                ? Array(8).fill(0).map((_, i) => <tr key={i}><td colSpan={6}><div className="h-10 shimmer rounded" /></td></tr>)
                : calls.map((c) => (
                  <tr key={c.id} className="cursor-pointer" onClick={() => openTranscript(c)}>
                    <td>
                    ...
                    <td className="text-[#8888aa]">{c.business_name}</td>
                    <td>
                    ...
                    <td>
                      <div className="flex items-center gap-1">
                        {c.recording_url && (
                          <button onClick={(e) => { e.stopPropagation(); downloadRecording(c.id); }}
                            className="btn-secondary btn-sm p-1.5" title="Download recording">
          ...
          {selectedCall ? (
            <>
              <div className="mb-4">
                ...
                <p className="text-xs text-[#555570] mt-1">{selectedCall.caller_number} — {formatDuration(selectedCall.duration_seconds)}</p>
              </div>
                  ...
                  : transcript.map((t, i) => (
                    <div key={i} className={`flex gap-2 ${t.role === 'agent' ? 'flex-row-reverse' : ''}`}>
                      <div className="text-xs px-3 py-2 rounded-xl max-w-[85%]"
                        style={{
                          background: t.role === 'agent' ? 'rgba(108,71,255,0.15)' : '#16161f',
                          color: t.role === 'agent' ? '#c4b5fd' : '#e8e8f0',
                        }}>
                        {t.text || t.message}
                      </div>
                    </div>
